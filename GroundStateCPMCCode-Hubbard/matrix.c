...
void jacobireal(double *matrix, double *eigenvalues, double *eigenvectors, int_st ist) {
     ...
     b=(double *)calloc(ist.n_sites,sizeof(double)); 
     z=(double *)calloc(ist.n_sites,sizeof(double)); 

     dzero_vec(eigenvalues,ist.n_sites); 
     dzero_vec(eigenvectors,ist.n_sites_sq); 

     /*Numerical Recipes Jacobi Routine*/
     for (ip=1;ip<=ist.n_sites;ip++) {
                for (iq=1;iq<=ist.n_sites;iq++) eigenvectors[(ip-1)*ist.n_sites+(iq-1)]=0.0;
                eigenvectors[(ip-1)*ist.n_sites+(ip-1)]=1.0;
        }
        for (ip=1;ip<=ist.n_sites;ip++) {
                b[ip-1]=eigenvalues[ip-1]=matrix[(ip-1)*ist.n_sites+(ip-1)];
                z[ip-1]=0.0;
        }
        for (i=1;i<200;i++) {
                sm=0.0;
                for (ip=1;ip<=ist.n_sites-1;ip++) {
                        for (iq=ip+1;iq<=ist.n_sites;iq++)
                                sm += fabs(matrix[(ip-1)*ist.n_sites+(iq-1)]);
                }
                if (sm == 0.0) {
                        free(b); 
                        free(z);
                ...
                if (i < 4)
                        tresh=0.2*sm/((float) ist.n_sites_sq);
                else
                        tresh=0.0;
                for (ip=1;ip<=ist.n_sites-1;ip++) {
                        for (iq=ip+1;iq<=ist.n_sites;iq++) {
                                g=100.0*fabs(matrix[(ip-1)*ist.n_sites+iq-1]);
                                if (i > 4 && (float)(fabs(eigenvalues[ip-1])+g) == (float)fabs(eigenvalues[ip-1])
                                        ...
                                        matrix[(ip-1)*ist.n_sites+iq-1]=0.0;
                                else if (fabs(matrix[(ip-1)*ist.n_sites+iq-1]) > tresh) {
                                        h=eigenvalues[iq-1]-eigenvalues[ip-1];
                                        if ((float)(fabs(h)+g) == (float)fabs(h))
                                                t=(matrix[(ip-1)*ist.n_sites+(iq-1)])/h;
                                        else {
                                                theta=0.5*h/(matrix[(ip-1)*ist.n_sites+iq-1]);
                                        ...
                                        s=t*c;
                                        tau=s/(1.0+c);
                                        h=t*matrix[(ip-1)*ist.n_sites+iq-1];
                                        z[ip-1] -= h;
                                        z[iq-1] += h;
                                        eigenvalues[ip-1] -= h;
                                        eigenvalues[iq-1] += h;
                                        matrix[(ip-1)*ist.n_sites+iq-1]=0.0;
                                        for (j=1;j<=ip-1;j++) {
                                           g=matrix[(j-1)*ist.n_sites+ip-1];
                                           h=matrix[(j-1)*ist.n_sites+iq-1];
                                           matrix[(j-1)*ist.n_sites+ip-1]=g-s*(h+g*tau);
                                           matrix[(j-1)*ist.n_sites+iq-1]=h+s*(g-h*tau);
                                        }
                                        for (j=ip+1;j<=iq-1;j++) {
                                                g=matrix[(ip-1)*ist.n_sites+j-1];
                                                h=matrix[(j-1)*ist.n_sites+iq-1];
                                                matrix[(ip-1)*ist.n_sites+j-1]=g-s*(h+g*tau);
                                                matrix[(j-1)*ist.n_sites+iq-1]=h+s*(g-h*tau);
                                        }
                                        for (j=iq+1;j<=ist.n_sites;j++) {
                                                g=matrix[(ip-1)*ist.n_sites+j-1];
                                                h=matrix[(iq-1)*ist.n_sites+j-1];
                                                matrix[(ip-1)*ist.n_sites+j-1]=g-s*(h+g*tau);
                                                matrix[(iq-1)*ist.n_sites+j-1]=h+s*(g-h*tau);
                                        }
                                        for (j=1;j<=ist.n_sites;j++) {
                                                g=eigenvectors[(j-1)*ist.n_sites+ip-1];
                                                h=eigenvectors[(j-1)*ist.n_sites+iq-1];
                                                eigenvectors[(j-1)*ist.n_sites+ip-1]=g-s*(h+g*tau);
                                                eigenvectors[(j-1)*ist.n_sites+iq-1]=h+s*(g-h*tau);
                                        }
                                }
                        }
                }
